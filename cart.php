...
while ($row = mysqli_fetch_array($result)) {
        $sum += $row["Price"];
        $id = $row["id"] . ", ";
        echo "<tr><td>" . "#" . $row["id"] . "</td><td>" . $row["Name"] . "</td><td>" . $row["Price"] . " Руб. </td><td><a href='cart-remove.php?id={$row['id']}' class='remove_item_link'> Убрать</a></td></tr>";
    }
    $id = rtrim($id, ", ");
    echo "<tr><td></td><td>Всего</td><td>" . $sum . " Руб.</td><td><a href='success.php' class='btn btn-primary'>Подтвердить заказ</a></td></tr>";
    ?>
                            </tbody>
                            <?php
} else {
    echo "<div> <img src='images/emptycart.png' class='image-fluid' height='150' width='150'></div><br/>";
    echo "<div class='text-bold  h5'>Добавьте товары в корзину<div>";
